...
bool ADPWService::registerObjectCreator(const QSharedPointer<IObjectCreator> &creator)
{
    if (nullptr == creator)
        return false;
...
QSharedPointer<IObjectCreator> ADPWService::getObjectCreator(const QString &name) const
{
    auto iter = mCreatorMap.find(_key(name));
    if (iter == mCreatorMap.end())
        return nullptr;
...
QObject *ADPWService::createObjectByDomElement(const QDomElement &domElement, QObject *parent)
{
    ...
    QSharedPointer<IObjectCreator> pObjCreator = getObjectCreator(objName);
